...
#else
#ifdef DEBUG
    ifstream in_all("E:/test/vscode/wordle/wordle_words_all.txt");
    ifstream in("E:/test/vscode/wordle/wordle_words_ans.txt");
#else
    ifstream in_all("./wordle_words_all.txt"), in("./wordle_words_ans.txt");
#endif
    assert(in.is_open() && in_all.is_open());
    string buffer;
    while (in_all >> buffer)
        all.emplace_back(buffer);
    while (in >> buffer)
        solves.emplace_back(buffer);
#endif
}
...
bool get_input(string &word, int status[5])
{
    ...
    for (char &ch : word)
        ch = tolower(ch);
    scanf("%1d %1d %1d %1d %1d", status, status + 1, status + 2, status + 3, status + 4);
    getchar();
    bool solved = true;
    for (int i = 0; i < 5; ++i)
        if (status[i] != 2)
...
int get_all_solves(const string &word, int status[5], bool simulate = false)
{
    ...
    for (int i = 0; i < 5; ++i)
        ...
        case 2:
            ...
            break;
        case 1:
            not_in[i][word[i] - 'a'] = true;
            ++cnt[word[i] - 'a'];
            break;
        default:
            for (int j = 0; j < 5; ++j)
                if (correct[j] != word[i])
                    not_in[j][word[i] - 'a'] = true;
            break;
        }
    for (int i = 0; i < 26; ++i)
        if (cnt[i] > exist[i])
            exist[i] = cnt[i];
    vector<string> new_solves;
    for (const string &str : solves)
    {
        ...
        {
            char ch = str[i];
            if (correct[i] && ch != correct[i])
            {
                flag = false;
                break;
            }
            else if (not_in[i][ch - 'a'])
            {
                flag = false;
                break;
            }
...
void print_solves()
{
    ofstream out("./wordle_solves.txt");
    assert(out.is_open());
    for (const string &str : solves)
        out << str << endl;
}
...
int main(int argc, char *argv[])
{
    ...
    int ch;
    while ((ch = getopt(argc, argv, "hvn:")) != -1)
    {
        switch (ch)
        {
        case 'h':
        ...
        case 'n':
            ...
            printf("Set Recommand Threshold: %d\n", threshold);
            break;
        default:
